	...
	function newtable_table(t1,t2){
		...
		//user
 		function users(current_page,submenu_li_id,call_for,this_user_id){
				...
				//Form
				var f_title='First Name@r@t=  #Last Name@@t=   #UserName@r@t= #Employee Id / API Access Id@@t= #Profile Image@@t= #Email@r@t= #Phone@@t= #Area@r@t= #Address@@t= #User Type@r@t= #Email Validation@@t=#Select Branch@@t=#Commission Rate@@t='; 
				var f_type='input#input#input#input#file@single@../assets/img/team/#input#input#select@newTable~select * from zone_districts  order by zone_districts_id ASC#input#select@newTable~select * from tbl_users_userType  order by userType_id ASC#moreSettings@email_validate#selectMulti@newTable~select * from branch  order by branch_id ASC#input';
				var f_column='firstName#lastName#userName#employee_id#image#userEmail#userPhone#zone_districts_id#address#userType#tbl_users_userStatus#branch_id#commission_rate';
				var f_classes='';
				var f_class='custom_class';
				var f_placeh='';
				...
				var th='SL,Name,Email,Clients,Branch,Usertype,Username,Phone,Action#text-end,Active#text-end,Created@fs--1';
				var table_info='tbl_users#userID#desc#'+this_user_id;
				var table_functions='multiselect@button_delete,this_function@'+users.name; 
				var td='firstName#@+lastName#@,userEmail#@,#&select * from services_clients_branch where userID=userID**@[fs--0*,branch_id#{branch_id}[branch]&select * from branch where branch_id+branch_id#{branch_id}[branch_code]&select * from branch where branch_id**@[fs--0*,userType#{userType_id}&select * from tbl_users_userType where userType_id**@[fs--0*#,userName#@,userPhone,_edit_1, _mark_1,created#@[fs--1*';
		...
		function userRole(current_page,submenu_li_id,call_for,this_user_id){
				...
				var table=
				'SL➤ ➤' 
				+'User Role Name➤userType#@[fs--1*➤'
				+'Dashboard>>Accounts#text-end➤_mark_1{is_dashboard_account}#@[fs--1*➤'
				+'Dashboard>>Orders#text-end➤_mark_1{is_dashboard_orders}#@[fs--1*➤'
				+'Stock Branch#text-end➤_mark_1{is_stock_branch}#@[fs--1*➤'
				+'New Pickup#text-end➤_mark_1{is_pickup}#@[fs--1*➤'
				
				+'Search & Status>>Advanced Search#text-end➤_mark_1{is_update_status}#@[fs--1*➤'
				+'Search & Status>>Make Payment#text-end➤_mark_1{is_update_status_payment}#@[fs--1*➤'
				+'Search & Status>>Update Status#text-end➤_mark_1{is_update_status_update}#@[fs--1*➤'
				
				+'Quick Update>>Origin Branch#text-end➤_mark_1{is_quick_status}#@[fs--1*➤'
				+'Quick Update>>All Branch#text-end➤_mark_1{is_quick_status_all}#@[fs--1*➤'
				
				+'POD Upload#text-end➤_mark_1{is_scan_pod}#@[fs--1*➤'
				
				+'Billing>>View Billing#text-end➤_mark_1{is_billing}#@[fs--1*➤'
				+'Billing>>Approved Collection#text-end➤_mark_1{is_billing_app_col}#@[fs--1*➤'
				+'Billing>>Make Disbursed #text-end➤_mark_1{is_billing_dis}#@[fs--1*➤'
				+'Billing>>Approved Disbursed #text-end➤_mark_1{is_billing_app_dis}#@[fs--1*➤'
				
				+'Issue#text-end➤_mark_1{is_issue}#@[fs--1*➤'
				+'All Report>>Pickup Report#text-end➤_mark_1{is_all_report}#@[fs--1*➤'
				+'All Report>>Shipment Report#text-end➤_mark_1{is_all_report_ship}#@[fs--1*➤'
				+'All Report>>Data Entry Report#text-end➤_mark_1{is_all_report_data}#@[fs--1*➤'
				+'All Report>>Delivery Boy Report#text-end➤_mark_1{is_all_report_deli}#@[fs--1*➤'
				
				+'Setup & Configuration#text-end➤_mark_1{is_setup_conf}#@[fs--1*➤'
				+'✦Marchant type#text-end➤_mark_1{is_marchant}#@[fs--1*➤'				 
				+'✦Delivery Boy type#text-end➤_mark_1{is_delivery_boy}#@[fs--1*➤'
				+'✦All Branch Access#text-end➤_mark_1{is_all_branch}#@[fs--1*➤'
				
				
				+'Created➤created#@[fs--1*➤'
  				+'Active#text-end➤_mark_1{is_key}#@[fs--1*➤'
				...
				var table_info=table_name+'#'+name_sm+'_id#asc#'+this_user_id;	
		...
		function dashboard_call(current_page,submenu_li_id,call_for,this_user_id,parent_function,name_sm,name_cap){
		 ...
		 function tracking_status(current_page,i_tracking_status_li_id,call_for,this_user_id){
			    ...
			    +'Select Delivery Status@r@t=➤'+newtableMulti('i_delivery_status','1','')+'➤i_delivery_status_id	➤'
				;
				...
				+'Delivery Status➤'+newtable_table('i_delivery_status','')+'@[fs--1*➤'
				+common_creator
		 ...
		 function payment_type(current_page,i_payment_type_li_id,call_for,this_user_id){
				...
				+common_creator
 				+'Have Collection Amount?#text-end➤_mark_1{have_collection}#@[fs--1*➤'
 				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
				...
				+'Packaging Type Id➤i_packaging_type_id#@[fs--1*➤'
				+'Weight➤weight#@[fs--1*➤'
			 ...
			 function delivery_status(current_page,i_delivery_status_li_id,call_for,this_user_id){
				...
				var table_functions='this_function@'+parent_function;   
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click@hide,footer_insert@Insert';
		 ...
		 function return_cause(current_page,i_return_cause_li_id,call_for,this_user_id){
			 ...
			 function sms_template(current_page,i_sms_template_li_id,call_for,this_user_id){
				...
				+'Message➤message#@[fs--1*➤'
 				+common_creator
				+'Global Non-Mask#text-end➤_mark_1{is_nonmasing_global}#@[fs--1*➤' 
 				+'Available for General Sender#text-end➤_mark_1{have_general_sender}#@[fs--1*➤'
 				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
			 ...
			 function branch(current_page,branch_li_id,call_for,this_user_id){
				...
				var form=
				'<sectionequalBasic Information>Branch Name<sectionequalBasic Information>@r@t=➤input➤branch➤➤'
				+'Branch code@@t=➤input➤branch_code➤➤'
				+'Branch Group@r@t=➤'+newtableMulti('branch_group','1','')+'➤branch_group_id➤➤'
				+'Web@@t=➤input➤web➤➤'
				+'Name (In-charge)@@t=➤input➤cl➤➤'
				+'phone@@t=➤input➤phone➤➤'
				+'fax@@t=➤input➤fax➤➤'
				+'Email@@t=➤input➤email➤➤'
				 
				+'<sectionequalBranch Address>Address<sectionequalBranch Address>@@t=➤area➤address➤➤'
				+'Select Country @r@t=➤'+newtable('zone_countries','1','')+'➤zone_countries_id	➤➤'
				+'Select Divisions @r@t=➤'+newtable('zone_divisions','1','')+'➤zone_divisions_id➤➤'
				+'Select Districts @r@t=➤'+newtable('zone_districts','1','')+'➤zone_districts_id➤➤'
				+'Select Upazilas @r@t=➤'+newtable('zone_upazilas','1','')+'➤zone_upazilas_id'
				...
				var table_info='branch#branch_id#desc@branch_id#'+this_user_id+'##  where parent_branch IS  NULL';	 
				var table_functions='multiselect@button_delete,this_function@'+parent_function;  
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert@@@@'
				+'#tbl_users=userID^branch_id^Concerning Users:userName|User Name-userEmail|Email'
;
			 ...
			 function branch_delivery(current_page,branch_delivery_li_id,call_for,this_user_id){
				var name_sm='branch_delivery';
				var name_cap='Delivery Man Group';
				var table_name='branch';
				var parent_function=branch_delivery.name;
				
 
				var form=
				'<sectionequalBasic Information>Delivery Group Name<sectionequalBasic Information>@r@t=➤input➤branch➤➤'
				+'Delivery Group code@@t=➤input➤branch_code➤➤'
				+'Delivery Parent Group@r@t=➤select@newTable~select * from  branch  where  parent_branch IS NULL    order by  branch_id ASC~branch_id~branch➤parent_branch➤➤'
				+'Web@@t=➤input➤web➤➤'
				+'phone@@t=➤input➤phone➤➤'
				+'fax@@t=➤input➤fax➤➤'
				+'Email@@t=➤input➤email➤➤'
				+'Cl@@t=➤input➤cl➤➤'
				+'Sl@@t=➤input➤sl➤➤'
				+'<sectionequalDelivery Man Group Address>Address<sectionequalDelivery Man Group Address>@@t=➤area➤address➤➤'
				+'Select Country @r@t=➤'+newtable('zone_countries','1','')+'➤zone_countries_id	➤➤'
				+'Select Divisions @r@t=➤'+newtable('zone_divisions','1','')+'➤zone_divisions_id➤➤'
				+'Select Districts @r@t=➤'+newtable('zone_districts','1','')+'➤zone_districts_id➤➤'
				+'Select Upazilas @r@t=➤'+newtable('zone_upazilas','1','')+'➤zone_upazilas_id'		
				...
				var table=
				'SL➤ ➤'
				+'Delivery Man Group Name➤branch#@[fs--1*➤'
				+'Delivery Man Group Code➤branch_code#@[fs--1*➤'
				+'Delivery Man Parent Group➤'+newtable_table('branch','')+'@[fs--1*➤'  
				+'Web➤web#@[fs--1*➤'
				+'Address➤address#@[fs--1*➤'
				+'Phone➤phone#@[fs--1*➤'
  				+'Status#text-end➤_mark_1{is_active}#@[fs--1*➤'
				...
				var table_info='branch#branch_id#desc@branch_id#'+this_user_id+'## where parent_branch >0';	
				var table_functions='multiselect@button_delete,this_function@'+parent_function;  
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert@@@@'
				+'#tbl_users=userID^branch_delivery_id^Concerning Users:userName|User Name-userEmail|Email'
;
				basic_fun(f_title,f_type,f_classes,f_column,th,td,table_info,table_request,table_functions,current_page,branch_delivery_li_id,call_for,this_user_id,parent_function,name_sm,name_cap,table_name,f_functions); 
		}	
			 ...
			 function branch_group(current_page,branch_group_li_id,call_for,this_user_id){
			    ...
			    +'Responsible Person---Name+Designation+Phone+Details@@t=➤input_mul+4➤responsible_persion➤responsible_persion'
   				;
				...
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert@@@@'
				+'#branch=branch_group_id^branch_id^Concerning Branch:branch|Branch Name-branch_code|Branch Code-address|Address'
				;
			 ...
			 function zone_divisions(current_page,zone_divisions_lid,call_for,this_user_id){
		        ...
		        +'Country@r@t=➤'+newtable('zone_countries','1','')+'➤zone_countries_id'
				...
				+'Country➤'+newtable_table('zone_countries','')+'@[fs--1*➤'  
  				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
 				+common_creator
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
...
//Services & Clients
		 function services(current_page,services_lid,call_for,this_user_id){
				...
				+'Services Name➤services#@[fs--1*➤'
  				...
  				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
				+'Tracking url General Customer#text-end➤_mark_1{is_track_web}#@[fs--1*➤'
				+'Delivery OTP #text-end➤_mark_1{is_otp}#@[fs--1*➤'
 				+common_creator
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
		 ...
		 function services_clients_type(current_page,services_clients_type_lid,call_for,this_user_id){
 		 ...
 		 function services_clients	(current_page,services_clients_lid,call_for,this_user_id){
				...
				+'Discount Percent@r@n=➤input➤discount_percent	➤➤'
				...
				+'Address➤address#@[fs--1*➤'
				+'Non-Masking SMS#text-end➤_mark_1{is_nonmasking}#@[fs--1*➤' 
  				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
 				+common_creator
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
				...
				var table_info='services_clients#services_clients_id#asc@services_clients#'+this_user_id+'## where 1';	 
				var table_functions='multiselect@button_delete,this_function@'+parent_function;  
				var table_request= search_bar('services_clients_type services_clients_type','2');
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert@@@@'
			    +'#services_clients_branch=services_clients_branch_id^services_clients_id^Concerning Branch:services_clients_branch|Branch Name-address|Address-userID+tbl_users|login username'
				;
		 ...
		 function services_clients_general(current_page,services_clients_general_lid,call_for,this_user_id){
				var name_sm='services_clients_general';
				var name_cap='All Customer';
				var table_name='pickup';
				var parent_function=services_clients_general.name;
				
 	
				var form=
				'Name@r@t=➤input➤sender_name_6➤➤'
				+'Phone@r@t=➤input➤sender_phone_5	➤➤'
				+'Address@r@t=➤area➤sender_address_9'
 				;
				...
				var table=
				'SL➤ ➤'
				+'Name➤sender_name_6#@[fs--1*➤'
				+'Phone➤sender_phone_5#@[fs--1*➤'
				+'Address➤sender_address_9#@[fs--1*➤'
				...
				var table_info='pickup#id#asc@id#'+this_user_id+'## where sender_category_1=\'general_sender\' ';	
				var table_functions='multiselect@button_delete,this_function@'+parent_function;  
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert@@@@'
			   
				;				
				basic_fun(f_title,f_type,f_classes,f_column,th,td,table_info,table_request,table_functions,current_page,services_clients_general_lid,call_for,this_user_id,parent_function,name_sm,name_cap,table_name,f_functions); 
		}
	
 		 function services_clients_branch(current_page,services_clients_branch_lid,call_for,this_user_id){
				var name_sm='services_clients_branch';
				var name_cap='Client Branch';
				var table_name='services_clients_branch';
				var parent_function=services_clients_branch.name;
				
 	
				var form=
				'<sectionequalBranch Information>Branch Name<sectionequalBranch Information>@r@t=➤input➤services_clients_branch➤➤'
			    +'Select Clients@r@t=➤'+newtable('services_clients','1','')+'➤services_clients_id➤➤'
				+'Select User@@t=➤select@newTable~select * from tbl_users where userType=25 or userType=30  order by userID ASC~userName~userID➤userID➤➤'
				+'<sectionequalBranch Address>Address<sectionequalBranch Address>@@t=➤area➤address➤➤'
				+'Select Country @r@t=➤'+newtable('zone_countries','1','')+'➤zone_countries_id	➤➤'
				+'Select Divisions @r@t=➤'+newtable('zone_divisions','1','')+'➤zone_divisions_id➤➤'
				+'Select Districts @r@t=➤'+newtable('zone_districts','1','')+'➤zone_districts_id➤➤'
				...
				var table=
				'SL➤ ➤'
				+'Branch Name➤services_clients_branch#@[fs--1*➤'
			    +'Client Name➤'+newtable_table('services_clients','')+'@[fs--1*➤'  
				+'Address➤address#@[fs--1*➤'
 				+common_creator
 				+'Active#text-end➤_mark_1{is_active}#@[fs--1*➤'
 				+'Edit#text-end@fs--1➤_edit_able@[text-end';
				...
				var table_info='services_clients_branch#services_clients_branch_id#asc@services_clients_id#'+this_user_id+'## where 1';	
				var table_functions='multiselect@button_delete,this_function@'+parent_function;  
				var table_request= search_bar();
				var f_functions='button_expand@Update@add_new_button@click,footer_insert@Insert';
				basic_fun(f_title,f_type,f_classes,f_column,th,td,table_info,table_request,table_functions,current_page,services_clients_branch_lid,call_for,this_user_id,parent_function,name_sm,name_cap,table_name,f_functions); 
		}	
